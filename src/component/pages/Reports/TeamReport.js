...
export default function TeamReport(props) {
  ...
  const fetchTeamReport = async () => {
    try {
      let response = await getDataAxios(`team/teamsReportData/${CompanyId}`);
  ...
  const handleSearch = async (e) => {
    ...
    getTempTableData.map((item) => {
      if (
        ...
        (item.Manager &&
          item.Manager.toLowerCase().includes(e.target.value.toLowerCase()))
      ) {
        searchArr.push(item);
      }
    });
  ...
  const handleFilter = async () => {
    try {
      ...
      let response = await postDataAxios(
        `team/newPenalFilterTeams/${userCompanyId.company_id}`,
        body
      );
