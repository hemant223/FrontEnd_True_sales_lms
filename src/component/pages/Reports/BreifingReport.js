...
export default function BreifingReport(props) {
  ...
  const fetchBriefingData = async () => {
    let response = await getDataAxios(`briefings/newPenalDisplay/${CompanyId}`);
    if (response.status) {
      setBriefingData(response.result);
  ...
  const handleFilter = async () => {
    try {
      ...
      let response = await postDataAxios(
        `briefings/newPenalDisplayFilter/${userCompanyId.company_id}`,
        body
      );
  ...
  const showEmployee = (i) => {
    ...
    try {
      ...
      created_at = moment(getBriefingData[i].created_at).format(
        "DD/MM/YYYY HH:mm a"
      );
    } catch (e) {
      title = "";
    ...
    return (
      <tr>
        <td> {title} </td>
        <td style={{ width: 280 }}> {description} </td>
        <td> {teamname} </td>
        <td>
          {" "}
          {from} to {to}
