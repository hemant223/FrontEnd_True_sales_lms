...
export default function ManagerTaskReport(props) {
  ...
  const fetchTaskReport = async () => {
    try {
      let response = await getDataAxios(
        `task/managerPenalTaskReport/${CompanyId}/${userData.id}`
      );
      if (response.status) {
  ...
  const handleFilter = async () => {
    try {
      ...
      let response = await postDataAxios(
        `task/managerPenalFilterTaskReport/${userCompanyId.company_id}/${userData.id}`,
        body
      );
  ...
  const showEmployee = (i) => {
    ...
    try {
      ...
      created = moment(getTaskReport[i].TaskDateTime).format(
        "DD/MM/YYYY hh:mm a"
      );
      ...
      task_added_date = moment(getTaskReport[i].AddedOn).format(
        "DD/MM/YYYY hh:mm a"
      );
    } catch (e) {
      name = "";
                              ...
                              <div class="row ">
                                ...
                                >
                                  ...
                                  <CSVLink
                                    ...
                                    filename={`${userData.name} manager task_Report.csv`}
                                  >
                                    <button
                                      type="button"
