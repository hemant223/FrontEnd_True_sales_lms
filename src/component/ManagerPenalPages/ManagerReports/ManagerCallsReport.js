...
export default function ManagerCallsReport(props) {
  ...
  const fetchCallsReport = async () => {
    try {
      let response = await getDataAxios(
        `calls/managerPenalCallsReport/${CompanyId}/${userData.id}`
      );
      if (response.status) {
        var arr = [];
  ...
  const fetchSalesExecutive = async () => {
    try {
      let CSRResponse = await getDataAxios(
        `usersR/AllCSRFetchForManagerPenal/${CompanyId}/${userData.id}`
      );
      ...
      console.log("error in fetch Sales Executive", e);
    }
  };
  ...
  const handleFilter = async () => {
    try {
      ...
      let response = await postDataAxios(
        `calls/managerPenalFilterCallsReport/${userCompanyId.company_id}/${userData.id}`,
        body
      );
  ...
  const showEmployee = (i) => {
    ...
    try {
      ...
      username = getCallsReport[i].UserName;
      customer = getCallsReport[i].CustomerName;
      created = moment(getCallsReport[i].CallDateTime).format(
        "DD/MM/YYYY hh:mm a"
      );
      duration = getCallsReport[i].duration;
      usernote = getCallsReport[i].user_note;
      disposition = getCallsReport[i].disposition;
      callStatus = getCallsReport[i].call_status;
      unknownNumber = getCallsReport[i].unknown_number;
      call_case_number = getCallsReport[i].case_number;
                              ...
                              <div class="row ">
                                ...
                                >
                                  ...
                                  <CSVLink
                                    ...
                                    filename={`${userData.name} ManagerPenal Calls_Report.csv`}
                                  >
                                    <button
                                      type="button"
                                      class="btn btn-info btn-sm"
