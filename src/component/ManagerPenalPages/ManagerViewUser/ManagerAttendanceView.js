...
export default function AttendanceView(props) {
  ...
  var reduxxStore = useSelector((state) => state.cart);
  var userrData = reduxxStore.UserDetail;
  ...
  const fetchUserAttendance = async () => {
    try {
      ...
      // console.log("response data fetchUserAttendance", response);
  ...
  const handleFilter = async () => {
    try {
      ...
      if (GraphFilterResponse.status == true) {
        ...
        swal({
          title: `Somewthing went wrong in attendance data.`,
          icon: "error",
          button: "ok",
        });
      }
