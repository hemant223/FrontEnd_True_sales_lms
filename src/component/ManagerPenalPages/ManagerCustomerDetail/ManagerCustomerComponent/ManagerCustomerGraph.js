...
export default function ManagerCustomerGraph(props) {
  // console.log("ManagerCustomerGraph Data->", props.startdate, props.enddate);
  ...
  const TotalCalls = async () => {
    try {
      ...
      let result = await postDataAxios(
        `customers/displayfilter/${props.ManagerCustomerInfo.id}/${props.ManagerCustomerInfo.company_id}`,
        body
      );
  ...
  const handleCurrentMonth = async () => {
    try {
      let body = { startDate: props.startdate, endDate: props.enddate };
      let result = await postDataAxios(
        `calls/Filtercurrentmonth/${props.ManagerCustomerInfo.id}/${props.ManagerCustomerInfo.company_id}`,
        body
      );
