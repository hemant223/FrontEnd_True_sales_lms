...
import AddManagerTask from "../AddManagerTask/AddManagerTask";
...
export default function ManagerTask(props) {
  ...
  const fetchAllTask = async () => {
    try {
      var result = await getDataAxios(
        `task/managerPenalTaskList/${UserData.company_id}/${UserData.id}`
      );
      // console.log("Result of Task Display in manager Penal", result.data);
      if (result.status) {
        setAllTasks(result.resultt[0]);
        setTempTableData(result.resultt[0]);
        setAllTasksExcelDownload(result.ExcelData[0]);
        setLoading(false);
      } else {
        swal({
  ...
  const fetchAllTaskExcelDownload = async () => {
    var result = await getDataAxios(
      `task/managerPenalTaskListExcelDownload/${UserData.company_id}/${UserData.id}`
    );
  ...
  const handleFilter = async () => {
    try {
      ...
      if (response.status == true) {
        ...
        //   console.log("in Else of Customer filter");
        swal({
          title: `Something went wrong.`,
          icon: "error",
          button: "ok",
